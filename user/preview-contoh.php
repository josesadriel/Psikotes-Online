...
<body>
    ...
    if (isset($_GET['id_subtes'])) {
        $getDataSubtes = mysqli_query($db, "SELECT * FROM `tbl_contohsoal` INNER JOIN `tbl_subtes` ON tbl_contohsoal.subtes = tbl_subtes.nama_subtes WHERE tbl_subtes.id_subtes = '$_GET[id_subtes]' LIMIT 1");
            ...
            <div class="container mt-1">
                <div class="card my-auto">
                    <div class="card-header d-flex">
                        <div class="pr-2 flex-fill"><?= $_GET['jenis_tes'] . ' - ' . $dataSubtes['nama_subtes'] ?></div>
                    </div>
                    <?php
                    if (isset($_GET['id_subtes'])) {
                        // paging
                        $limit = 1; // berapa banyak soal yang akan ditampilkan
                        $hitungSoal = mysqli_query($db, "SELECT COUNT(tbl_contohsoal.id_soal) AS 'jumlah' FROM `tbl_contohsoal` INNER JOIN `tbl_jenistes` ON tbl_contohsoal.jenis_tes = tbl_jenistes.nama_tes INNER JOIN `tbl_subtes` ON tbl_subtes.nama_subtes = tbl_contohsoal.subtes WHERE tbl_jenistes.nama_tes = '$_GET[jenis_tes]' AND tbl_subtes.id_subtes = '$_GET[id_subtes]'");
                        ...
                        $listSoal = mysqli_query($db, "SELECT * FROM `tbl_contohsoal` WHERE jenis_tes = '$_GET[jenis_tes]' AND subtes = '$dataSubtes[nama_subtes]' ORDER BY added_at ASC LIMIT $start_from, $limit");
                            ...
                            <div class="card-body">
                                <form action="#" method="POST">
                                    ...
                                    <div id="selanjutnya" style="display:none">
                                        <a href="?page=<?= ($page + 1) . "&jenis_tes=" . $_GET['jenis_tes'] . "&id_subtes=" . $_GET['id_subtes']; ?>" class="btn btn-primary">Selanjutnya</a>
                                    </div>
                                    ...
                                    <div id="kembaliKerjakan" style="display:none">
                                        <a onClick="window.close();" class="btn btn-primary">Selesai</a>
                                    </div>
                                </form>
                                <?php
